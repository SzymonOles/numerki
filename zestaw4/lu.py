...
if(__name__ == "__main__"):
    ...
    parser.add_argument(
        "matrix", help="Matrix entered following the format example: 1 2 3 , 4 5 6 , 7 8 9")
    args = parser.parse_args()

    a = np.array(
        list(map(lambda r: list(map(float, r.split())), args.matrix.split(','))))
    ...
    while(con):
        ...
        if(len(command.split()) == 4 and command.split()[0] == "add"):
            ...
            mod = addRow(rowFrom, [(mul if r == rowTo else 0.0)
                         for r in range(a.shape[0])])
            modInv = addRow(
                rowFrom, [(-mul if r == rowTo else 0.0) for r in range(a.shape[0])])
            ...
            mod = mulRow([(mul if r == row else 1.0)
                         for r in range(a.shape[0])])
            modInv = mulRow([((1.0 / mul) if r == row else 1.0)
                            for r in range(a.shape[0])])
            ...
            row = int(command.split()[1]) - 1
            mul = float(command.split()[2])
            mod = mulRow([((1.0 / mul) if r == row else 1.0)
                         for r in range(a.shape[0])])
            modInv = mulRow([(mul if r == row else 1.0)
                            for r in range(a.shape[0])])
            nicePrint(mod, "<multiplying row> = ")
            nicePrint(modInv, "<multiplying row>^-1 = ")
        elif(len(command.split()) == 3 and command.split()[0] == "swap"):
            row = int(command.split()[1]) - 1
            row2 = int(command.split()[2]) - 1
            tempa = np.array(a[row2])
            tempb = np.array(b[row2])
            a[row2]=a[row]
            b[row2]=b[row]
            a[row]=tempa
            b[row]=tempb
            print("<swapping rows>: " + str(row) + " "+ str(row2))
            nicePrint(a, description="A = ")
